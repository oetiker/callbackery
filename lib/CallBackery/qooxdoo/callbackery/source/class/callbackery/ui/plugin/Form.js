    ...
    members: {
        ...
        _reconfForm: function(triggerField){
            if (this._reConfFormInProgress) {
                this._reconfPending.push({field: triggerField});
                return;
            }
            ...
            rpc.callAsyncSmartBusy(function(pluginConfig) {
                if (pluginConfig){
                    that._reConfFormHandler(pluginConfig.form);
                }
                that._reConfFormInProgress = false;
                if (that._reconfPending.length > 0) {
                    var pending = that._reconfPending.shift();
                    that._reconfForm(pending.field);

...
qx.Class.define("callbackery.ui.form.Auto", {
     ...
     */
    construct : function(structure, layout, formRenderer, pluginName) {
        this.base(arguments);
        this._settingData = 0;
        ...
        let plugin = qx.core.Id.getQxObject(pluginName);
//        console.log('plugin=', plugin, ', Id=', qx.core.Id.getInstance().getQxObject(plugin));
        let oldForm = qx.core.Id.getInstance().getQxObject(pluginName + '/Form');
        if (oldForm) {
//            console.log('Deleting old form',  pluginName);
            plugin.removeOwnedQxObject('Form');
            delete oldForm;
        }
        if (plugin) {
            plugin.addOwnedQxObject(form, 'Form');
            console.log('Registered form as', qx.core.Id.getAbsoluteIdOf(form));
        }
        else {
            console.warn('Parent plugin not found:', pluginName);
        }
        this._ctrl = {};
...
//                console.log('s=', s);
                ...
                qx.core.Id.getInstance().register(header, 'MyHeader');
                header.setQxObjectId('MyHeader');

...
qx.Class.define("callbackery.ui.plugin.Action", {
    ...
    construct : function(cfg,buttonClass,layout,getFormData, plugin) {
        ...
        this.addListener('appear', () => {
            let buttonMap = this._buttonUrlActionMap;
            let config    = callbackery.data.Config.getInstance();
            Object.entries(buttonMap).forEach(data => {
                const [key, value] = data;
                const urlAction = value.urlAction;
                const button = value.button;
                const urlValue = config.getUrlConfigValue(urlAction.key);
                if (urlValue == urlAction.value) {
                    button.execute();
                }
            });
        }, this);
                ...
                switch (btCfg.action) {
                    ...
                    case 'download':
                        ...
                        if (btCfg.key){
                            this._buttonMap[btCfg.key]=button;
                            if (btCfg.urlAction) {
                                this._buttonUrlActionMap[btCfg.key] = {
                                    button    : button,
                                    urlAction : btCfg.urlAction
                                };
                            }
                        }
                        if (btCfg.buttonSet) {

...
sub register {
  ...
  my $preprocess = $conf->{preprocess} || 'CallBackery';
  ...
  $app->routes->any(
      ...
      $module =~ s/\//\:\:/g;      
      if ($module eq 'CallBackery::Cfg'){
          $html = _pod_to_html( CallBackery::Config->new()->pod() );
      }
      else {
          my $path;
          if ($module eq 'CallBackery::LocalGuide'){
...
CallBackery::DocPlugin - Extopus Documentation Plugin

...
to limit accessibility. By default plugins are not accessible you have numeric UID or the word C<__CONFIG>.
...
Walks a hash/array structure and removes all occurances of the given key.
...
take the data from the plugin screen and process them
...
receive current data for plugin screen content
...
validate user suplied data prior to acting on it
...
sub getConfigValue {
    my $self = shift;
    my $key = shift;
    my $value = $self->user->db->fetchValue('config',{id => $key},'value');
...
sub setConfigValue {
    ...
    $self->user->db->updateOrInsertData('config',{
        id=> $key,

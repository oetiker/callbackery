package CallBackery::RpcService;
...
sub login { ## no critic (RequireArgUnpacking)
    ...
    my $db = CallBackery::Database->new(app=>$self->app);
    my $userData = $db->fetchRow('user',{login=>$login});
    return {} if not $userData;
    if ($userData->{user_password} and $password 
        and hmac_sha1_sum($password) eq $userData->{user_password} ){
        $self->user->userId($userData->{user_id});
        return {
            sessionCookie => $self->user->makeSessionCookie()
        }
    }
